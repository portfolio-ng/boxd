            ...
            "options": [
                {
                    "shrt": "-t \"",
                    "full": "--title \"",
                    "helper": "rule title options :: <br />\"extreme prejudice\" : remove all \ninstances of string, -o -e -r unnecesssary <br /> \"string \nreplace\", replace a string depending on parameters  <br \n/>\"prefix replace\", replaces a prefix to a string <br \n/>\"suffix replace\", replaces a suffix to a string"
                },
                {
                    "shrt": "-o \"",
                    "full": "--rgxop \"",
                    "helper": "opening regular expression :: <br />must be placed in ( ) <br /> ..\"(\b[^a-z])\""
                },
                {
                    "shrt": "-e \"",
                    "full": "--rgxen \"",
                    "helper": "ending regular expression :: <br />must be placed in ( ) <br /> ..\"(?=[^a-z])\""
                },
                {
                    "shrt": "-m \"",
                    "full": "--rgxmd \"",
                    "helper": "listing of flagd strings in an array :: <br />must be placed in ( )\n with each value seperated by a pipe, |, character <br /> \n..(no|not|never|nothing|nope)"
                },
                {
                    "shrt": "-s \"",
                    "full": "--strar \"",
                ...
                },
                {
                    "shrt": "-l \"",
                    "full": "--lngsp \"",
                    "helper": "longest split :: <br />longest possible string length to cover all necessary replacements<br /> ..\"40\""
                },
                {
                    "shrt": "-v \"",
                    "full": "--rvflg \"",
                    "helper": "review flag :: <br />flagged key to initiate review of the boxes contents<br />.. \"\\W\""
                },
                {
                    "shrt": "-d \"",
                    "full": "--dspsn \"",
                ...
                },
                {
                    "shrt": "-x \"",
                    "full": "--extnt \"",
                    "helper": "extent of back word"
                },
                {
                    "shrt": "-k \"",
                    "full": "--brkpt \"",
                    "helper": "flag to break from pattern"
                },
                {
                    "shrt": "-n \"",
                    "full": "--bcknm \"",
                    "helper": "how many words to look back when using backword"
                },
                {
                    "shrt": "-p \"",
                    "full": "--patln \"",
                    "helper": "length of the pattern"
                },
                {
                    "shrt": "-u \"",
                    "full": "--patou \"",
                    "helper": "where to find the pattern <\br> firstfirst, firstlast, lastfirst, lastlast"
                }
            ]
        }
    }
...
function fleshrule(thisbox) {
  ...
  while (i < optsnum) { // do until all opts have been processed
    ...
    var keyfromfull = dataobj.meta.parserule.options[optarrval].full.substring(2, 7);
...
function pattern(thisrule, manipulated, thisbox) {
  ...
  var manofimp = manipulated.ofimport;
  var manorimp = manipulated.orofimport;
  if (manipulated.backcheck) {
  if (manipulated.backcheck.length > thisrule.real.bcknm) {
  if (thisrule.real.bcknm > 1) {
    var midstring = manorimp.substring(manofimp[0].length, manorimp.length - manofimp[1].length)
  } else {
    var midstring = "";
  }
    if (manofimp[1].length > pattlen) {
      //if firstfirst
			switch ( pattype) {
				case "firstlast":
			    if (manofimp[0].substring(0, pattlen) == manofimp[1].substring(manofimp[1].length-1-pattlen, manofimp[1].length-1)) {
            var change = manofimp[0] + midstring + manofimp[1];
          } else {
            var change = manofimp[0] + midstring + manofimp[0].substring(0, pattlen);
						manipulated.cursdisp = manipulated.cursdisp-pattlen;
          }
				break;
				case "lastfirst":
			    if ( manofimp[0].substring(manofimp[0].length-1-pattlen, manofimp[0].length-1) == manofimp[1].substring(0, pattlen)) {
            var change = manofimp[0] + midstring + manofimp[1];
          } else {
            var change = manofimp[0] + midstring + manofimp[0].substring(manofimp[0].length-1-pattlen,manofimp[0].length-1);
          }
				break;
				case "lastlast":
			    if ( manofimp[0].substring(manofimp[0].length-1-pattlen, manofimp[0].length-1) == manofimp[1].substring(manofimp[1].length-1-pattlen, manofimp[1].length-1)) {
            var change = manofimp[0] + midstring + manofimp[1];
          } else {
            var change = manofimp[0] + midstring + manofimp[0].substring(manofimp[0].length-1-pattlen,manofimp[0].length-1);
						manipulated.cursdisp = manipulated.cursdisp-pattlen;
				...
				break;
			  default: { //firstfirst
			    if (manofimp[0].substring(0, pattlen) == manofimp[1].substring(0, pattlen)) {
            var change = manofimp[0] + midstring + manofimp[1];
          } else {
            var change = manofimp[0] + midstring + manofimp[0].substring(0,pattlen);
          }
      ...
      var startpatt = manofimp[0].substring(0, manofimp[1].length);
      if (startpatt == manofimp[1]) {
        var change = manofimp[0] + midstring + manofimp[1]
      } else {
        var change = manofimp[0] + midstring;
      }
    }
  } else {
    var change = manofimp.join("");
  }

  manipulated = repositioning(thisbox, manipulated, change);
	return manipulated;
	} else { // if the user attempts to start with punctuation
	manofimp= thisbox.value;
...
function checkundy(rule,def){
    if (rule == "")
      rule = def; // review on newline, or carriage return
return rule;
} //checkundy(), checks if rule value is undefined
...
function getdefaults(thisrule) {
  ...
  case "calculate":
      for (var ruleopt in thisrule.real) {
        ...
        case "title":
          break;
          //list to avoid default deletion
        case "rvflg":
          thisrule.real.rvflg = checkundy(thisrule.real.rvflg, "(\r)"); // review on newline, or carriage return
          break;
        case "lngsp":
          thisrule.real.lngsp = checkundy(thisrule.real.lngsp, "line"); // calculate uses the line longest split
          break;
        case "dspsn":
          thisrule.real.dspsn = checkundy(thisrule.real.dspsn, "new"); //new or inline, where to display the solution
          break;
        default:
          {
            delete thisrule.real[ruleopt]
          }
        } //switch (ruleopt)
      } //for key in rule
    break;
  case "pattern":
      for (var ruleopt in thisrule.real) {
        ...
        case "title":
          ...
          //list to avoid default deletion
        case "rvflg":
            thisrule.real.rvflg = checkundy(thisrule.real.rvflg,"(\\W+)"); // review on anti word pattern
          break;
        case "lngsp":
            thisrule.real.lngsp = checkundy(thisrule.real.lngsp, "backword"); // patterns use the backword longest split
          break;
        case "extnt":
            thisrule.real.extnt = checkundy(thisrule.real.extnt, "full"); // full or break, determining the extent to which the pattern should continue beyond a break point
          break;
        case "brkpt":
            thisrule.real.brkpt = checkundy(thisrule.real.brkpt, "(\r|\.|,|!|?)"); // if extent is break, this is the breat point... default: newline, fullstop, comma, exclamation point, and question mark
          break;
        case "bcknm":
            thisrule.real.bcknm = checkundy(thisrule.real.bcknm, 1); // number of units to look back for pattern 
          break;
        case "patln":
            thisrule.real.patln = checkundy(thisrule.real.patln, 1); // length of pattern, corresponds to number of characters 
          break;
        case "patou":
            thisrule.real.patou = checkundy(thisrule.real.patou, "firstfirst"); // firstfirst or firstlast or lastfirst or lastlast, determining where, the french ou, to find the pattern
          break;
        default:
          {
            delete thisrule.real[ruleopt]
  ...
  default: // in case flex defaults were forgotten
    {
      for (var ruleopt in thisrule.real) {
        ...
        case "rvflg":
          ...
          break;
        case "lngsp":
            thisrule.real.lngsp = checkundy(thisrule.real.lngsp, "40"); // longest split is 40 characters
          break;
        case "rgxop":
            thisrule.real.rgxop = checkundy(thisrule.real.rgxop, "(\\b)"); // boundary preceding the desired string set
          break;
        case "rgxen":
            thisrule.real.rgxen = checkundy(thisrule.real.rgxen, "(?=\\W+)"); // anti word pattern 
          break;
        case "rgxmd":
            thisrule.real.rgxmd = checkundy(thisrule.real.rgxmd, "gi"); // global and case insensitive
          break;
        default:
          {
            delete thisrule.real[ruleopt]
  ...
  return thisrule;
} // getdefaults(), function to fill in defaults for rules
